...
//    private static final int RIM_SIZE = 3;
    ...
    public SlideSwitch(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        color_theme = a.getColor(R.styleable.slideswitch_themeColor,
                DEFAULT_COLOR_THEME);
        color_back_theme = a.getColor(R.styleable.slideswitch_backColor,
                Color.WHITE);
        isOpen = true;
        shape = SHAPE_CIRCLE;
        a.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        if (shape == SHAPE_RECT) {
            ...
            canvas.drawRect(backRect, paint);
            frontRect.set(frontRect_left, borderWidth,
                    frontRect_left + backRect.height() - 2 * borderWidth,
            ...
            // 变化中的背景主色，变化范围覆盖区域包括背景
            paint.setColor(color_theme);
            paint.setAlpha(alpha);
            canvas.drawRoundRect(backBorderCircleRect, backBorderRadius, backBorderRadius, paint);

            // 前景圆圈的边框
            frontBorderRect.set(frontRect_left, 0,
                    frontRect_left + backBorderRect.height(),
            ...
            frontRect.set(frontRect_left + borderWidth, borderWidth,
                    frontRect_left + backBorderRect.height() - borderWidth,
                    backBorderRect.height() - borderWidth);
